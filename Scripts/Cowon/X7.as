...
function InitializeLauncher(Void)
{
    ...
    LoadConfiguration();
    if (_global.GetFirmwareVersion() == X7_OVERSEAS && ext_fscommand2("GetSysRegion") == -1)
    {
...
_global.X7_DOMESTIC = 1;
_global.X7_OVERSEAS = 2;
_global.X7_DOMESTIC_DMB = 3;
_global.X7_DOMESTIC_VOKHAN = 4;
...
_global.DisplayAudioOut = function (forceUpdate)
{
    ...
    if (forceUpdate == 1 || prevMute != muteState || preAudiOut != currentAudioOut)
    {
        ...
        _level1.MCCon.MCInfobar.MCVol.MCVolIcon.gotoAndStop(preAudiOut + 1);
        _level1.MCCon.MCInfobar.MCVol.gotoAndStop(muteState + 1);
...
_global.gfn_DispVolume = _global.DisplayVolume = function (forceUpdate)
{
    ...
    if (forceUpdate == 1 || prevVolume != currentVolume)
    {
        ...
        if (_level1.MCCon.MCInfobar.MCVol._currentframe == 1)
        {
            _global.CommonSetDigitMC(_level1.MCCon.MCInfobar.MCVol, currentVolume);
        } // end if
    } // end if
};
...
_global.GetFirmwareVersion = function (Void)
{
    if (firmwareVersion == -1)
    {
        ...
        {
            firmwareVersion = _global.X7_DOMESTIC_DMB;
        } // end if
